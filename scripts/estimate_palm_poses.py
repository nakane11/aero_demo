...
#        下限・上限を (-1.00, -0.60) に取っていたときは「水平よりだいぶ下」
#        まで満点近くになり、親指が下向きの不自然な差し出し姿勢 (例:
#        generate_random_human_poses.py --seed 0 の human_022/033/045/047)
#        が他の特徴量だけで OFFER_SCORE_MIN を超えて通過してしまっていた。
#        同シードの 100 件で計測すると、実際に採用された差し出し手の
#        thumb_roll は -0.089 以上にしか分布しない一方、上記の不自然な
#        4 件 (と human_020/076/084) は -0.55 以下に固まっており、明確な
#        ギャップがある。このギャップに合わせて下限を -0.20 まで引き上げ、
#        上限も -0.30 から 0.30 (ほぼ水平) まで広げることで、水平よりわずか
#        に下向きの自然な差し出し (thumb_roll ~0.06-0.1) は満点近くを保った
#        まま、明らかに下向きな姿勢だけを罰する。
